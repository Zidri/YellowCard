...
function Edit2() {
  ...
  const generatePreview = async (pdfFile) => {
    ...
    const res = await fetch("https://yellowcard-backend-1.onrender.com/preview_pdf", {
      method: "POST",
      body: formData
    });
  ...
  return (
    <div style={{ display: "flex", height: "100vh" }}>
      ...
      <div className="sidebar">
        <Link title="Back" className="ed2head1" to="/edit">
          <FaArrowLeft />
        </Link>

        <button title="Save" className="ed2head2" onClick={exportPDF}>
          <FaFileExport />
        </button>
        ...
        <div className="align-buttons">
          <button
            className="edcursor"
            title="Split"
            onClick={() => setIsSelectingSplit(true)}
          >
            <FaCut />
          </button>
                  ...
                  <Draggable key={idx} draggableId={String(idx)} index={idx}>
                    {(provided) => (
                      <div
                        ...
                        onClick={() =>
                          isSelectingSplit && togglePageSelect(idx)
                        }
                        style={{
                          position: "relative",
                          width: "60%",
